      ...
      <div class="footerBg">
        <div class="cooperationNetworkPerson">
          ...
          <div id="content" style="position: relative;">
            ...
            <div class="cooperationList" v-show="model1">
              <Tabs
                type="card"
                :animated="false"
...
export default {
  ...
  data() {
    return {
      ...
      normalWidth: null,
      // 示例数据
      perId1: "9b7e7eeefe254fad9a78957d256687b8",
      person_name1: "何国强：西北工业大学",
      model3: "20"
    };
  },
  watch: {
  ...
  methods: {
    ...
    executeQuery() {
      if (this.queryName == "" && this.model2 == "") {
        ...
        this.$axios
          .post("/coop/network", this.$qs.stringify(data))
          .then(res => {
            let myChart = null;
            if (res.data.code == 200) {
          ...
          // });
              ...
              if (personNetworkData.length !== 0) {
                ...
                for (let i = 1; i < nodesData.length; i++) {
                  ...
                  if (temp.value >= 10 && temp.value <= 20) {
                    this.normalWidth = 1.5;
                  }
                  if (temp.value >= 20 && temp.value <= 30) {
                    this.normalWidth = 2;
                  }
                  if (temp.value >= 30 && temp.value <= 50) {
                    this.normalWidth = 2.5;
                  }
                  if (temp.value >= 50 && temp.value <= 80) {
                    this.normalWidth = 3;
                  }
                  if (temp.value >= 80 && temp.value <= 110) {
                    this.normalWidth = 4;
                  }
                  if (temp.value >= 110 && temp.value <= 110) {
                    this.normalWidth = 5;
                  }
                  temp.lineStyle = {
                    normal: {
                ...
                titleData =
                  ...
                  `</span>` +
                  `</p><p class="promptTitle">最多可显示前100人的合作关系</p>` +
                  ...
                  tooltip: {
                    formatter: function(params) {
                      if (params.dataType == "edge") {
                        return "合作数量为：" + params.value;
                      }
                      if (params.dataType == "node") {
                        return (
                          params.data.label + "：" + params.data.organization
                        );
                      }
                    }
                  },
                ...
                if (nodesData.length === 1) {
                  ...
                  for (let i = 1; i < cooNum; i++) {
                    if (nodesData[i].value >= maxPersonNum) {
                      ...
                      maxPersonName = nodesData[i].label;
                    }
                  }
                  if (cooNum == 0) {
                    resultContent +=
                    ...
                    resultContent +=
                      ...
                      "</span></p>";
                    resultContent +=
                      '<p class="promptTitle">鼠标放在节点上可查看科研人员的合作次数，点击连线可查看合作明细，点击专家节点可进入详情页。</p>';
                  } else {
                    resultContent +=
                      '<p class="FirstTitle"><span>' +
                      perName +
                ...
                myChart.on("click", params => {
                  if (params.dataType === "edge") {
                    this.model1 = true;
                    this.personId2 = nodesData[params.data.target].person;
                    this.markTabChange();
