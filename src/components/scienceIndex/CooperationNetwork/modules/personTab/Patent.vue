  ...
  <div>
    <div v-if="!loading">
      <div class="sc-reward" v-for="(item,index) in contentData" :key="index">
        <h3
          class="paperName"
          @click="patentSetPath(item.name?item.name:'未知','/patentdetails',item.patentId)"
        ...
        <div class="sc-div">
          ...
          <Row>
            <i-col span="8">
              <label>专利权人:</label>
              <span>{{item.patentee.organizationName == ''?'未知':item.patentee.organizationName}}</span>
            </i-col>
            <i-col span="8">
              <label>申请日:</label>
...
export default {
  ...
  methods: {
    ...
    // 专利跳转
    patentSetPath(name, path, patentId) {
    ...
    // 作者跳转
    authorSetpath(pstentId) {
      this.$emit("handle-details", patentId);
    }
  }
};
</script>
