  ...
  <div>
    <div v-if="!loading">
      <div class="sc-reward" v-for="(item,i) in contentData" :key="i+'j'">
        <h3 class="paperName" @click="projectSetPath(item.name?item.name:'未知','/projectdetails',item.projectId)">{{item.name?item.name:'未知'}}</h3>
      <div class="sc-div">
        <Row>
          <i-col span="12">
            <label>项目开始时间:</label>
        ...
        <Row>
          <i-col span="12" >
            <label>项目负责人:</label>
            <span class="charge"
              @click.stop="handleDetails(item.charge.personId)"
            >{{item.charge.personName?item.charge.personName:'未知'}}</span>
          </i-col>
          <i-col span="12">
            <label>项目牵头单位:</label>
            <span class="leadUnit" @click="organSetpath(subItem.organizationName,'/organdetails',subItem.organizationId)" v-for="(subItem,i) in item.leadUnit" :key="i" >{{subItem.organizationName == 'null'?'未知':subItem.organizationName}}</span>
          </i-col>
        </Row>
        <Row>
          <i-col span="24">
