  ...
  <div>
    <div v-if="!loading">
      <div class="sc-reward" v-for="(item,index) in contentData" :key="index">
        <h3
          class="paperName"
          @click="monographSetPath(item.name?item.name:'未知','/monographdetails',item.monographId)"
...
export default {
  ...
  methods: {
    ...
    // 专著跳转
    monographSetPath(name, path, paperId) {
    ...
    // 机构跳转
    organSetpath(name, path, organizationId) {
      let positionArr = this.$store.state.variable.position.slice(0, 2);
      positionArr.push({
        pathName: "专著（" + name + "）",
        routerName: "/"
    ...
    // 作者跳转
    authorSetpath(monographId) {
      this.$emit("handle-details", monographId);
    }
  }
};
</script>
