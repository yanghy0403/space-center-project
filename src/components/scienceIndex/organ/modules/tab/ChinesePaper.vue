      ...
      <div class="sc-reward" v-for="(item,index) in contentData" :key="index">
        ...
        <div class="sc-div">
          ...
          <Row>
            <i-col span="12">
              <label>期刊：</label>
              <span class="journal" @click="journalSetPath(item.journal.journalName,'/journaldetails',item.journal.journalId)">{{item['journal'].journalName}}</span>
            </i-col>
            <i-col span="12">
              <label>发表时间：</label>
...
export default {
  ...
  methods: {
    ...
    // 期刊跳转
    journalSetPath(name, path, journalId) {
let positionArr = this.$store.state.variable.position.slice(0, 2);
      positionArr.push({
        pathName: "科研人员" + name + "）",
        routerName: "/"
...
.authorArr:hover,.journalId:hover {
  color: #1843A3;
