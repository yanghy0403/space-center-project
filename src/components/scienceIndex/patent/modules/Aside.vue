  ...
  <div class="sc-warp">
    <aside>
      ...
      <span>点击检索栏下的重置按钮返回上级</span>
        ...
        <Panel key="1">
            <label>专利类型</label>
          <ul slot="content" v-if="patentTypeData.length !== 0">
            <li v-for="(item,index) in patentTypeData" :key="index + 1" @click.stop="handleClick(item,'专利类型',false)">
              <p>
        ...
        <Panel key="2">
            <label>专利状态</label>
          <ul slot="content" v-if="patentStatusData.length !== 0">
            <li v-for="(item,index) in patentStatusData" :key="index + 1" @click.stop="handleClick(item,'专利状态',false)">
              <p>
                <Icon type="md-arrow-dropright" />
                {{item.name}}
              </p>
...
export default {
  ...
  watch: {
    data(news, old) {
      this.newData = news;
      this.patentTypeData = news[0]['patentType'];
      this.patentStatusData = news[1]['patentStatus'];
    },
    additional(news, old) {
      this.addData = news;
    }
